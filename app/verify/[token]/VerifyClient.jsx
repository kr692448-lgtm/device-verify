...
/* ── Sun SVG ── */
function SunIcon({ size=14, color }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
      <circle cx="8" cy="8" r="3.2" stroke={color} strokeWidth="1.4"/>
      {[0,45,90,135,180,225,270,315].map((deg,i)=>{
        const r=(deg*Math.PI)/180;
        const x1=8+5.2*Math.cos(r), y1=8+5.2*Math.sin(r);
...
/* ── Moon SVG ── */
function MoonIcon({ size=14, color }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
...
function Avatar({ name, color }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontSize:20, fontWeight:900, color, letterSpacing:1,
...
/* ── USER STRIP ── */
function UserStrip({ name, userId, color, dark }) {
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",flexDirection:"column",gap:5,minWidth:0}}>
        <span style={{
          color: dark ? "#fff" : "#0a0f1e",
          fontSize:17, fontWeight:800, lineHeight:1, letterSpacing:0.3,
          fontFamily:"'SF Mono','Fira Code',monospace",
      ...
      <svg width={S} height={S} style={{position:"absolute",inset:0,overflow:"visible"}}>
        ...
        {spinning && <>
          <circle cx={cx} cy={cx} r={r1} fill="none" stroke={c.main} strokeWidth="2"
            ...
            style={{transformOrigin:`${cx}px ${cx}px`,animation:"spin 1.8s linear infinite"}} filter="url(#f1)"/>
...
/* ── PROGRESS ── */
function Progress({ pct, color, dark }) {
  return (
    <div>
      <div style={{height:1.5,background: dark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.08)",borderRadius:2,overflow:"hidden"}}>
        <div style={{
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginTop:5}}>
        <span style={{color: dark?"rgba(255,255,255,0.18)":"rgba(0,0,0,0.3)",fontSize:7.5,letterSpacing:1.5}}>SCAN PROGRESS</span>
...
export default function VerifyClient({ token, session }) {
  ...
  async function runScan(){
    ...
    try{
      ...
      setState("verified");setStatusText("ACCESS GRANTED");setSubText("Identity confirmed — you may now proceed");addLog("AUTH: Verification complete");
  ...
  /* ── theme-aware helpers ── */
  const bg       = dark ? "linear-gradient(160deg,#03040e 0%,#060818 50%,#04060f 100%)" : "linear-gradient(160deg,#e8edf8 0%,#eef2fc 50%,#e6ebf7 100%)";
  const gridClr  = dark ? "rgba(0,207,255,0.03)" : "rgba(0,100,200,0.04)";
  const cardBg   = dark ? "rgba(5,7,18,0.96)"    : "rgba(255,255,255,0.97)";
  const cardBdr  = dark ? "rgba(255,255,255,0.07)": "rgba(0,0,0,0.08)";
  const cardShad = dark
    ? `0 60px 140px rgba(0,0,0,0.95),0 0 80px ${C.main}0c,inset 0 1px 0 rgba(255,255,255,0.05)`
    : `0 30px 80px rgba(0,0,0,0.1),0 0 60px ${C.main}12`;
  const topBarBg = dark ? "rgba(255,255,255,0.01)"  : "rgba(255,255,255,0.8)";
  const divider  = dark ? "rgba(255,255,255,0.05)"  : "rgba(0,0,0,0.06)";
  const botBarBg = dark ? "rgba(0,0,0,0.35)"        : "rgba(240,243,255,0.9)";
  const labelClr = dark ? "rgba(255,255,255,0.18)"  : "rgba(0,0,0,0.28)";
  const subClr   = dark ? "rgba(255,255,255,0.35)"  : "rgba(0,0,0,0.45)";
  const logBg    = dark ? "rgba(0,0,0,0.45)"        : "rgba(235,240,255,0.7)";
  const logBdr   = dark ? "rgba(255,255,255,0.04)"  : "rgba(0,0,0,0.05)";
  const logOld   = dark ? "rgba(255,255,255,0.18)"  : "rgba(0,0,0,0.22)";
  const logNew   = dark ? "rgba(255,255,255,0.75)"  : "rgba(0,0,0,0.72)";
  const toggleBg = dark ? "rgba(255,255,255,0.05)"  : "rgba(0,0,0,0.06)";
  const toggleBd = dark ? "rgba(255,255,255,0.1)"   : "rgba(0,0,0,0.12)";
  const toggleCl = dark ? "rgba(255,255,255,0.55)"  : "rgba(0,0,0,0.5)";
  const blob1    = dark ? "rgba(0,207,255,0.06)"    : "rgba(0,120,255,0.05)";
  const blob2    = dark ? "rgba(120,0,255,0.05)"    : "rgba(80,0,200,0.04)";

  return (
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        html,body{height:100%;overflow-x:hidden;transition:background 0.4s;}
      ...
      {/* bg */}
      <div style={{position:"fixed",inset:0,zIndex:0,background:bg,transition:"background 0.4s"}}/>
      <div style={{
        position:"fixed",inset:0,zIndex:0,
        backgroundImage:`linear-gradient(${gridClr} 1px,transparent 1px),linear-gradient(90deg,${gridClr} 1px,transparent 1px)`,
        backgroundSize:"48px 48px",animation:"noiseDrift 8s linear infinite",transition:"background-image 0.4s",
      }}/>
      ...
      <div style={{position:"fixed",top:14,right:14,zIndex:10}}>
        <button
          ...
          {dark ? <SunIcon size={13} color={toggleCl}/> : <MoonIcon size={13} color={toggleCl}/>}
          {dark ? "LIGHT" : "DARK"}
        </button>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          transition:"background 0.4s,border-color 0.3s,box-shadow 0.6s",
        }}>
          ...
          <div style={{
            ...
            <span style={{color:labelClr,fontSize:8,letterSpacing:3,transition:"color 0.4s"}}>
              NXTZEN · SECURE VERIFY
            </span>
            <div style={{width:7,height:7,borderRadius:"50%",background:C.main,boxShadow:`0 0 10px ${C.main}`,animation:"pulse 2s infinite"}}/>
          </div>

          {/* body */}
          <div style={{padding:"22px 20px 20px"}}>
            <UserStrip name={userName} userId={userId} color={C.main} dark={dark}/>

            <div style={{marginBottom:22}}><Ring state={state}/></div>
            ...
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <div style={{color:subClr,fontSize:9.5,letterSpacing:0.5,lineHeight:1.7,transition:"color 0.4s"}}>
                {subText}
              </div>
            </div>

            <div style={{marginBottom:16}}>
              <Progress pct={pct} color={C.main} dark={dark}/>
            </div>

            <div style={{
              background:logBg,border:`1px solid ${logBdr}`,
              borderRadius:6,padding:"10px 12px",minHeight:74,marginBottom:14,
              transition:"background 0.4s,border-color 0.4s",
            }}>
              {logs.length===0
                ?<span style={{color:labelClr,fontSize:8.5,letterSpacing:1}}>&gt; AWAITING SCAN...</span>
                :logs.map((l,i)=>(
                  <div key={i} style={{
                    color:i===logs.length-1?logNew:logOld,
                    fontSize:8.5,letterSpacing:0.2,lineHeight:2.1,transition:"color 0.3s",
                  }}>
            ...
            {shortFp&&(
              <div style={{
                ...
                background:`${C.main}07`,border:`1px solid ${C.main}20`,borderRadius:6,
              }}>
                <span style={{color:labelClr,fontSize:7.5,letterSpacing:2}}>DEVICE ID</span>
            ...
            {state==="conflict"&&(
              <div style={{
                ...
                background:"rgba(255,51,102,0.05)",border:"1px solid rgba(255,51,102,0.15)",borderRadius:6,
              }}>
                <div style={{color:"#ff3366",fontSize:9,letterSpacing:2,fontWeight:700,marginBottom:5}}>CONFLICT DETECTED</div>
                <div style={{color:subClr,fontSize:8.5,lineHeight:1.9}}>
                  This device fingerprint is already bound to a different account. Each device may only link to one account.
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginTop:8,paddingTop:7,borderTop:"1px solid rgba(255,51,102,0.1)"}}>
                  <span style={{color:labelClr,fontSize:7.5,letterSpacing:1}}>CODE</span>
          ...
          <div style={{
            ...
            <span style={{color:labelClr,fontSize:7.5,letterSpacing:1,transition:"color 0.4s"}}>SHA-256 · AES-256 · TLS 1.3</span>
            <span style={{color:labelClr,fontSize:7.5,letterSpacing:1,transition:"color 0.4s"}}>NXTZEN ENGINE</span>
          </div>
        </div>
      </div>
    </>
